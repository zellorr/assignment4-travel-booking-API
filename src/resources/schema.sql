...
CREATE TABLE hotel_bookings (
                                ...
                                CONSTRAINT fk_hotel_bookings_booking
                                    FOREIGN KEY (booking_id) REFERENCES bookings(id) ON DELETE CASCADE
);
...
CREATE TABLE travel_packages (
                                 ...
                                 created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
                                 CONSTRAINT fk_travel_packages_customer
                                     FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE RESTRICT,
...
CREATE TABLE package_bookings (
                                  ...
                                  CONSTRAINT fk_package_bookings_package
                                      FOREIGN KEY (package_id) REFERENCES travel_packages(id) ON DELETE CASCADE,
                                  CONSTRAINT fk_package_bookings_booking
                                      FOREIGN KEY (booking_id) REFERENCES bookings(id) ON DELETE CASCADE
);
...
SELECT
    ...
    ROUND(
            COALESCE(SUM(b.total_price), 0) * (1 - p.discount_percentage / 100.0),
            2
    ) AS total_after_discount,
    ROUND(
            COALESCE(SUM(b.total_price), 0) * (p.discount_percentage / 100.0),
            2
    ) AS discount_amount
